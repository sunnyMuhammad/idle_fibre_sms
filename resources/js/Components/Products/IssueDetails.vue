...
<template>
  <div v-if="modal" class="fixed inset-0 z-50 flex items-center justify-center">
    <div id="modal-content" class="bg-white w-[1100px] max-w-full rounded-xl shadow-2xl p-6 relative print:w-full print:shadow-none print:p-0 h-[500px] overflow-auto">
      ...
      <h1 class="text-sm font-bold text-center  pb-2">Report Date: {{props.fromDate?props.fromDate:'-'}} to {{props.toDate?props.toDate:'-'}}</h1>
      <!-- Table -->
        ...
        <table class="w-full border border-gray-300 text-sm">
          <thead class="bg-gray-100">
            <tr>
              ...
              <th class="px-4 py-2 border text-center ">Issue Qty</th>
              <th class="px-4 py-2 border text-center ">Machine Name</th>
              <th class="px-4 py-2 border text-center ">Issue Date</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in props.issueProduct" :key="index" class="hover:bg-gray-50 border-1 print:border-2">
              <td class="px-4 py-2 border text-center">{{ index + 1 }}</td>
              <td class="px-4 py-2 border text-center">{{ item.product.name }}</td>
              <td class="px-4 py-2 border text-center">{{ item.unit }}</td>
              <td class="px-4 py-2 border text-center">{{ item.machine_name }}</td>
